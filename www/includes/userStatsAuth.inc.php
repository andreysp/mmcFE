...
function hashGraphs($graph = NULL, $userId = NULL, $interval = "48", $type = "area") {
 ...
 echo '<script type="text/javascript" src="js/jquery.visualize-bargraphs.js"></script>';
	...
	if($graph == "mine") {
		...
		// hashrates
		echo "<tbody><tr><th scope='row'>" .$userInfo->username. " (Kh/s)</th>";
		$count = 0;
	...
	if ($graph == "pool") {
		...
		// hashrates
		echo "<tbody><tr><th scope='row'>Pool (Mh/s)</th>";
		$count = 0;
		while ($row = mysql_fetch_array($last24hhrQ, MYSQL_ASSOC)) {
	...
	if ($graph == "both") {
		...
		echo "</tr><tr><th scope='row'>Pool (Mh/s)</th>";
		// Pool
		$count = 0;
...
function financialGraphs($graph = NULL, $userId = NULL, $interval = "30", $type = "area") {
 global $userInfo;
	...
	if($graph == "mine") {
		...
		// amounts
		echo "<tbody><tr><th scope='row'>" .$userInfo->username. " (LTC)</th>";
